...
class GifticonListFragment : BaseFragment<FragmentGifticonListBinding>(
    ...
    private fun initUI() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            gifticonViewModel.selectedGifticon.collect { uiState ->
                when(uiState) {
                    is SelectedGifticonState.Selected -> {
                        Log.d(TAG, "initUI: ${uiState.gifticon}")
                        findNavController().navigate(R.id.action_gifticonListFragment_to_gifticonDetailFragment)
                    }
                    is SelectedGifticonState.Error -> {
                        Log.d(TAG, "initUI: ${uiState.message}")
                    }
                    is SelectedGifticonState.Initial -> {
                        Log.d(TAG, "initUI: Initial")
                    }
                }
            }
        }
    ...
    private fun initAdapter() {
        ...
        gifticonRVAdapter.itemClickListener = object : GifticonRVAdapter.ItemClickListener {
            override fun onClick(view: View, data: Gifticon, position: Int) {
                gifticonViewModel.selectGifticon(data)
            }
        }
    }
}