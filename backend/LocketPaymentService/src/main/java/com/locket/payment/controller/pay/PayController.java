...
public class PayController {
    ...
    @Operation(
            summary = "💳 내 카드 목록 조회",
            description = "✅ 사용자 ID를 기반으로 등록된 모든 카드를 조회합니다.\n💰 각 카드의 이번 달 총 결제 금액(`monthlyUsage`)과 카드 혜택(`benefits`)도 함께 제공합니다."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "🟢 조회 성공"),
            @ApiResponse(responseCode = "401", description = "🟡 인증 실패", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
            @ApiResponse(responseCode = "404", description = "🔴 사용자의 카드가 존재하지 않음", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    ...
    @Operation(summary = "📊 월간 총 결제 금액 조회", description = "사용자 ID, 연도, 월을 기반으로 해당 월의 총 결제 금액을 반환합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "🟢 조회 성공"),
            @ApiResponse(responseCode = "401", description = "🟡 인증 실패", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
            @ApiResponse(responseCode = "500", description = "🟠 서버 내부 오류", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
